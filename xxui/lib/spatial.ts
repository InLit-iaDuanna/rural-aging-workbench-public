...
export type Road = {id:string;a:string;b:string;width:number|null;source:string;access:string;confirmed:boolean;steps:boolean;lit:boolean};
...
export type Snapshot = {id:string;name:string;time:string;actor:string;reason:string;nodes:Node[];roads:Road[];buildings:Building[];hazards:Hazard[];facility:Facility|null;config:Config;image:string;scale:number;layout:LayoutItem[];goal:string;budget:string;preserve:string;version:number};
...
export type RouteResult = {nodes:string[];roads:string[];length:number;cost:number;events:{road:string;kind:string;category:string;severity:string;source:string;advice:string}[];unknown:number;reachable:boolean};
...
export const defaultConfig:Config={role:'拄杖',vision:'一般视力',familiarity:'熟悉',carrying:'无',environment:'日常',start:'n1',end:'n5',habit:['n1','n2','n4','n5']};
export function emptySpatial(address=''):Spatial{return {mode:'empty',image:'',scale:.5,crs:'演示局部平面坐标',address,locationConfirmed:false,nodes:[],roads:[],buildings:[],hazards:[],facility:null,config:{...defaultConfig},history:[],audit:[],version:0,layout:[],goal:'增设遮阳、休息和照明，让老人可以安心停留。',budget:'低',preserve:'保留树木、建筑、道路与原有出入口'};}
...
export function roadEvents(s:Spatial,r:Road,c:Config){
 ...
 if(c.environment==='夜间'&&!r.lit)events.push({road:r.id,kind:'夜间照明不足',category:'条件性问题',severity:c.vision==='低视力'?'高':'中',source:'用户道路属性',advice:'夜间实走核查照明连续性。'});
...
function roadCost(s:Spatial,r:Road,c:Config,lowRisk:boolean){const a=s.nodes.find(n=>n.id===r.a)!,b=s.nodes.find(n=>n.id===r.b)!;const len=distance(a,b)*s.scale;const events=roadEvents(s,r,c);return len+(lowRisk?events.reduce((v,e)=>v+(e.category==='数据缺口'?35:e.severity==='高'?180:90),0)*(c.role==='耐力较弱'?1.2:1)*(c.carrying==='无'?1:1.15)+(c.familiarity==='不熟悉'?12:0):0);}
export function calculateRoute(s:Spatial,c:Config,preference:'shortest'|'safe'|'habit'):RouteResult{
 const failed:RouteResult={nodes:[],roads:[],length:0,cost:0,events:[],unknown:0,reachable:false};
 ...
 return {nodes:routeNodes,roads:routeRoads,length:roads.reduce((v,r)=>v+roadCost(s,r,c,false),0),cost:roads.reduce((v,r)=>v+roadCost(s,r,c,true),0),events,unknown:events.filter(e=>e.category==='数据缺口').length,reachable:true};
}
